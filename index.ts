...
class TuiSession {
  ...
  screenHtml(): string {
    try {
      return this.serializer.serializeAsHTML({ scrollback: 0, includeGlobalBackground: false });
    } catch {
  ...
  async apply(action: GameAction, goal?: string, narration?: string) {
    ...
    if (action.type === "press_sequence") for (const key of action.keys.slice(0, 16)) { this.proc.write(bytesForKey(key)); await sleep(80); }
    if (action.type === "wait") await sleep(Math.max(0, Math.min(30, action.seconds)) * 1000);
    ...
    await writeTurnFile(record, this.agentState());
  }
...
async function request(pathname: string, init?: RequestInit): Promise<any> {
  const res = await fetch(`${serverUrl}${pathname}`, { ...init, headers: { "content-type": "application/json", ...(init?.headers ?? {}) } });
  if (!res.ok) throw new Error(`${res.status} ${res.statusText}: ${await res.text()}`);
  return res.json();
}
const state = () => request("/api/agent_state");
const command = (action: GameAction, goal?: string, narration?: string) => request("/api/agent_command", { method: "POST", body: JSON.stringify({ action, goal, narration }) });
...
export default function(pi: ExtensionAPI) {
  ...
  pi.registerCommand("tui", {
    description: "Run/drive a TUI app. Usage: /tui run [--port 7777] [--cols 140] [--rows 40] <command> | stop | status",
    handler: async (args, ctx) => {
      const argv = parseArgs(args || "");
      const sub = argv.shift();
      if (!sub || sub === "status") { ctx.ui.notify(statusLine(), "info"); return; }
      if (sub === "stop") { runningLoop = false; turnInFlight = false; pendingPrompt = false; session?.dispose(); session = null; ctx.ui.notify("TUI driver stopped", "info"); return; }
      if (sub !== "run") { ctx.ui.notify("Usage: /tui run [--port 7777] [--cols 140] [--rows 40] <command>", "error"); return; }
  ...
  pi.on("session_shutdown", async () => { runningLoop = false; session?.dispose(); });
